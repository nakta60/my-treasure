...
# 시스템 방정식 정의
def system_dynamic(t, vars, Q_x, Q_y, r_p):
...
for i in range(Q_x.shape[0]):
    for j in range(Q_x.shape[1]):
        ...
        # 적분 수행
        solution = solve_ivp(system_dynamic, t_span, initial_conditions, args=(Q_x[i, j], Q_y[i, j], 1), method='BDF', t_eval=time_points)
...
plt.xlabel('$Q_x$')
plt.ylabel('$Q_y$')
plt.title('Patterns of Species Coexistence')
plt.show()
